...
class Fangsheng extends CI_Controller {
	...
	public function index()
	{
		...
		{
			# code...
			$this->fangsheng_model->delete(rtrim($this->input->post('deleteid'), ','));
		}
		$total= $this->db->count_all('hhs_fangsheng');
		$page_config['perpage']=10;   //每页条数
		$page_config['part']=2;//当前页前后链接数量
		$page_config['url']='fangsheng/index';//url
		...
		$data['news'] = $this->fangsheng_model->get(($offset),$page_config['perpage']);
		$attributes = array('id' => 'indexform');
		$hidden = array('deleteid' => '','mymethod'=>'delete');
		$data['formurl'] = form_open('fangsheng/index', $attributes,$hidden);
		$data['arrayleft'] = $this->sidebar;
		$this->load->view('templates/head', $data);
		$this->load->view('templates/menu');
		$this->load->view('templates/left',$data);
		$this->load->view('fangsheng/index', $data);
	...
	public function create()
	{
		...
		if ($this->form_validation->run() === FALSE)
		{
			...
			$this->load->view('templates/menu');
			$this->load->view('templates/left',$data); 
			$this->load->view('fangsheng/create',$data);
			$this->load->view('templates/footer');
		}
		else
		{
			$this->fangsheng_model->create();
			$data['base'] = $this->config->item('base_url');
			$data['showmsg']="添加成功";
	...
	public function show()
	{
		...
		$data=$this->fangsheng_model->getOne($id);
		$data['base'] = $this->config->item('base_url');
		$this->load->view('templates/head', $data);
		$this->load->view('fangsheng/show', $data); 
		$this->load->view('templates/footer'); 
	...
	public function update()
	{
		...
		$this->load->view('templates/menu');
		$this->load->view('templates/left',$data); 
		$this->load->view('fangsheng/update',$data);
		$this->load->view('templates/footer');
	}
	public function commit()
	{
		...
		switch ($this->uri->segment(3)) {
			case 'update':
				# code...
				$this->fangsheng_model->update($type_id);
				$data['showmsg']="更新成功";
...
/* End of file fangsheng.php */
/* Location: ./application/controllers/fangsheng.php */
?>