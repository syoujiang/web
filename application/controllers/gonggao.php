...
class Gonggao extends CI_Controller {
	...
	public function index()
	{
		...
		$page_config['url']='gonggao/index';//url
		...
		$data['news'] = $this->gonggao_model->get(($offset),$page_config['perpage']);
		$attributes = array('id' => 'indexform');
		$hidden = array('deleteid' => '','mymethod'=>'delete');
		$data['formurl'] = form_open('gonggao/index', $attributes,$hidden);
		$data['arrayleft'] = $this->sidebar;
		$this->load->view('templates/head', $data);
		$this->load->view('templates/menu');
		$this->load->view('templates/left',$data);
		$this->load->view('gonggao/index', $data);
	...
	public function create()
	{
		...
		if ($this->form_validation->run() === FALSE)
		{
			...
			$this->load->view('templates/menu');
			$this->load->view('templates/left',$data); 
			$this->load->view('gonggao/create',$data);
			$this->load->view('templates/footer');
		}
		else
		{
			$this->gonggao_model->create();
			$data['base'] = $this->config->item('base_url');
			$data['showmsg']="添加成功";
	...
	public function show()
	{
		...
		$data=$this->gonggao_model->getOne($id);
		$data['base'] = $this->config->item('base_url');
		$this->load->view('templates/head', $data);
		$this->load->view('gonggao/show', $data); 
		$this->load->view('templates/footer'); 
	...
	public function update()
	{
		...
		$this->load->view('templates/menu');
		$this->load->view('templates/left',$data); 
		$this->load->view('gonggao/update',$data);
		$this->load->view('templates/footer');
	}
	public function commit()
	{
		...
		switch ($this->uri->segment(3)) {
			case 'update':
				# code...
				$this->gonggao_model->update($type_id);
				$data['showmsg']="更新成功";
...
/* End of file gonggao.php */
/* Location: ./application/controllers/gonggao.php */